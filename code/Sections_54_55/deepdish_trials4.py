...
h_conv1 = tf.nn.dropout(tf.nn.relu(bn_conv2d(x, W_conv1, is_training) + b_conv1), keep_prob)

W_conv2 = tf.get_variable("Wconv2", shape=[5, 5, 32, 32])
b_conv2 = tf.get_variable("bconv2", shape=[32])
h_conv2 = tf.nn.dropout(tf.nn.relu(bn_conv2d(h_conv1, W_conv2, is_training) + b_conv2), keep_prob)

W_conv3 = tf.get_variable("Wconv3", shape=[5, 5, 32, 32])
b_conv3 = tf.get_variable("bconv3", shape=[32])
h_conv3 = tf.nn.dropout(tf.nn.relu(bn_conv2d(h_conv2, W_conv3, is_training) + b_conv3), keep_prob)

W_conv4 = tf.get_variable("Wconv4", shape=[5, 5, 32, 32])
b_conv4 = tf.get_variable("bconv4", shape=[32])
h_conv4 = tf.nn.dropout(tf.nn.relu(bn_conv2d(h_conv3, W_conv4, is_training) + b_conv4), keep_prob)
h_pool4 = max_pool_2x2(h_conv4)

W_conv5 = tf.get_variable("Wconv5", shape=[5, 5, 32, 32])
b_conv5 = tf.get_variable("bconv5", shape=[32])
h_conv5 = tf.nn.dropout(tf.nn.relu(bn_conv2d(h_pool4, W_conv5, is_training) + b_conv5), keep_prob)
...
with tf.Session() as sess:
    ...
    for i in range(10000):
        ...
        _, loss_i = sess.run([train_step, cross_entropy], feed_dict={x: batch[0], y_: batch[1], keep_prob: 0.75, is_training:True})
...
#plt.legend(loc='lower right')
#
#plt.savefig('convnet_classificationaccuracy.png')
#plt.show()
#
#
plt.plot(loss_history[:400], 'o')
plt.title('Loss history for Epoch 1')
plt.xlabel('Iterations')
plt.ylabel('Loss')
plt.grid(True)
plt.legend(loc='lower right')

plt.savefig('convnet_losshist_epoch1.png')
plt.show()
